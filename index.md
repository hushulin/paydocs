...
# 回调地址
#### 介绍
> 当您收到付款时，IPN系统将通知您的服务器。这是将我们的付款集成到您的软件中以自动完成订单，统计，判断用户付款等等最简单的方法。它通过一个 HTTP POST 请求发送到您的服务器配置的URL上。我们请求的频率为1分钟1次，直到达到10次上限或您的服务器返回json里包含status="OK"（区分大小写）才停止请求。或者是在此订单结束时，也将终止该订单的请求。
#### 系统设置
