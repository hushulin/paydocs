...
# 技术参数
#### 交易相关
> 商家注册后，系统会为商家分配```20```个钱包地址池以供商家的客户使用，商家在登录后点击“获取地址”按钮同样消耗地址池里的钱包地址。所以这意味着，在订单规定的交易时间内（```15```分钟），最多产生交易```20```笔。超出后“获取地址”和API接口都将获取不到地址从而服务不可用。```15```分钟后，前面交易结束地址将得到释放，服务恢复可用。如果您有更高的并发需求，请联系官方进行扩大地址池容量或者进行嵌入开发。推荐嵌入开发彻底突破限制，商家自己掌控并发性能与数据。
